...
int main()
{
	...
	for(variance = 0.0; variance <= mean; variance += INCREASE_VARIANCE)
	{
		int longest = 0;
		int victories = 0;
		...
		printf("Guaranteed victories with 50%% confidence: %d\n",results[(unsigned int)(ITERATIONS / 2.0)]);
		printf("Guaranteed victories with 95%% confidence: %d\n",results[(unsigned int)(ITERATIONS / 20.0)]);
	}
